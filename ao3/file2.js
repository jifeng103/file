...
document.writeln('        // 如果已经访问过，移除指定元素');
document.writeln('        var login = document.getElementById("login");');
document.writeln('        if (login) login.innerHTML = "";'); // 移除 login 元素的内容
document.writeln('        var p = document.getElementsByTagName("p");');
document.writeln('        for (var i = 0; i < p.length; i++) {');
document.writeln('          if (p[i].className.includes("actions") && p[i].querySelector("a[href=\"/invite_requests\"]")) {');
document.writeln('            p[i].parentNode.removeChild(p[i]);'); // 移除包含特定链接的 p 标签
document.writeln('          }');
document.writeln('        }');
document.writeln('      }');
document.writeln('    };');
